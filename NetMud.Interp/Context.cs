...
namespace NetMud.Interp
{
    ...
    public class Context : IContext
    {
        ...
        /// <summary>
        /// The word captured for selecting the command
        /// </summary>
        public string OriginalCommandPhrase { get; private set; }
        ...
        public Context(string fullCommand, IActor actor)
        {
            ...
            Position = Actor.CurrentLocation.Clone(Actor.CurrentLocation.CurrentCoordinates);
        ...
        public void SeekInDirection(CommandPackage command, Type paramType, CommandParameterAttribute currentNeededParm)
        {
            ...
            while (parmDirWords > 0)
            {
                ...
                if (paramType == typeof(ITile) || paramType == typeof(IInanimate))
                {
                    ...
                    ITile tile = currentPosition.GetTile();
                    if (!x.IsBetweenOrEqual(0, 99) || !y.IsBetweenOrEqual(0, 99) || tile == null || tile.Type == null)
                    {
                        AccessErrors.Add("That is an invalid direction to interact with.");
                        return;
                    }

                    if (paramType == typeof(IInanimate))
                        thing = tile.TopContents();
                    else
                        thing = tile;
                }
            ...
            while (parmWords > 0)
            {
                ...
                switch (seekRange.Type)
                {
                    ...
                    case CommandRangeType.Touch:
                        //validObjects.AddRange(Position.GetTile().GetContents<T>().Where(ent => ((IEntity)ent).Keywords.Any(key => key.Contains(currentParmString))));
                        ...
                        //{
                        //    foreach (IContains thing in Position.CurrentZone.GetContents<T>().Where(ent => ent.GetType().GetInterfaces().Any(intf => intf == typeof(IContains))
        ...
        /// <summary>
        /// Find a parameter target on an entity that is a valid interaction with that entity
        /// </summary>
        /// <typeparam name="T">the system type of the entity</typeparam>
        /// <param name="commandType">the system type of the command</param>
        /// <param name="currentNeededParm">the conditions for the parameter we're after</param>
        /// <param name="hasContainer">does the command need a container to look for things in</param>
        /// <param name="seekRange">how far we can look</param>
        public void SeekInInteractions<T>(CommandPackage command, CommandParameterAttribute currentNeededParm, CommandRangeAttribute seekRange, bool hasContainer)
        {
            ITile tile = null;
            IEntity thing = null;
            bool useThing = false;

            //We should already have a subject or target set so we should have our target
            if (currentNeededParm.Usage == CommandUsage.Target)
            {
                thing = Subject as IEntity;

                if (thing == null)
                    tile = Subject as ITile;
                else
                    tile = thing.CurrentLocation.GetTile();
            }
            else if (currentNeededParm.Usage == CommandUsage.Supporting)
            {
                thing = Target as IEntity;

                if (thing == null)
                    tile = Target as ITile;
                else
                    tile = thing.CurrentLocation.GetTile();
            }
            ...
            if (internalCommandString.Count == 0)
                return;
            ...
            while (parmWords > 0)
            {
                ...
                IInteraction itemInteraction = null;

                //We need to try to interact with the object
                if (thing != null)
                {
                    useThing = true;
                    itemInteraction = thing.Interactions.FirstOrDefault(inte => inte.Name.Equals(currentParmString, StringComparison.InvariantCultureIgnoreCase));
                }

                if (itemInteraction == null)
                {
                    useThing = false;
                    itemInteraction = tile.Type.Interactions.FirstOrDefault(inte => inte.Name.Equals(currentParmString, StringComparison.InvariantCultureIgnoreCase));
                }
                ...
                if (itemInteraction != null)
                {
                    ...
                    {
                        IMobile dude = (IMobile)Actor;

                        if (itemInteraction.StaminaCost > 0 && dude.CurrentStamina < itemInteraction.StaminaCost)
                        {
                            AccessErrors.Add("You lack the stamina to use that.");
                            return;
                        }

                        if (itemInteraction.HealthCost > 0 && dude.CurrentHealth < itemInteraction.HealthCost)
                        {
                            AccessErrors.Add("You lack the stamina to use that.");
                            return;
                        }
                    ...
                    foreach (IActionCriteria criterion in itemInteraction.Criteria)
                    {
                        ...
                        switch (criterion.Target)
                        {
                            case ActionTarget.Self:
                                ...
                                {
                                    AccessErrors.Add(string.Format("YOU must have a quality of {0} between {1} and {2}.",
                                        criterion.Quality, criterion.ValueRange.Low, criterion.ValueRange.High));
                                    return;
                                }
                                break;
                            case ActionTarget.Tool:
                                if (Actor.GetType().GetInterfaces().Any(typ => typ == typeof(IContains)))
                                {
                                    IInanimate validTool = ((IContains)Actor).GetContents<IInanimate>().FirstOrDefault(ent => ent.Qualities.Any(key =>
                                            key.Type == QualityType.Aspect
                                             && (criterion.ValueRange.Low <= 0 || key.Value.IsBetweenOrEqual(criterion.ValueRange.Low, criterion.ValueRange.High))
                                            && key.Name.Equals(criterion.Quality, StringComparison.InvariantCultureIgnoreCase)));
                                    ...
                                    if (validTool == null)
                                    {
                                        AccessErrors.Add(string.Format("You must be carrying an item with a quality of {0} between {1} and {2}.",
                                            criterion.Quality, criterion.ValueRange.Low, criterion.ValueRange.High));
                                        return;
                                    }

                                    //Set the Tool to the one after the interaction
                                    switch (currentNeededParm.Usage + 1)
                                    {
                                        case CommandUsage.Supporting:
                                            Supporting = validTool;
                                            break;
                                        case CommandUsage.Subject:
                                            Subject = validTool;
                                            break;
                            ...
                            case ActionTarget.Tile:
                                if (useThing)
                                {
                                    if (!thing.Qualities.Any(key =>
                                             key.Type == QualityType.Aspect
                                             && (criterion.ValueRange.Low <= 0 || key.Value.IsBetweenOrEqual(criterion.ValueRange.Low, criterion.ValueRange.High))
                                             && key.Name.Equals(criterion.Quality, StringComparison.InvariantCultureIgnoreCase)))
                                    {
                                        AccessErrors.Add(string.Format("The {0} must have a quality of {1} between {2} and {3}.",
                                            thing.GetDescribableName(Actor), criterion.Quality, criterion.ValueRange.Low, criterion.ValueRange.High));
                                        return;
                                    }
                                }
                                else
                                {
                                    if (!tile.Qualities.Any(key =>
                                             key.Type == QualityType.Aspect
                                             && (criterion.ValueRange.Low <= 0 || key.Value.IsBetweenOrEqual(criterion.ValueRange.Low, criterion.ValueRange.High))
                                             && key.Name.Equals(criterion.Quality, StringComparison.InvariantCultureIgnoreCase)))
                                    {
                                        AccessErrors.Add(string.Format("The {0} must have a quality of {1} between {2} and {3}.",
                                            tile.Type.Name, criterion.Quality, criterion.ValueRange.Low, criterion.ValueRange.High));
                                        return;
                                    }
                                }
                                break;
                    ...
                    {
                        case CommandUsage.Supporting:
                            Supporting = itemInteraction;
                            break;
                        case CommandUsage.Subject:
                            Subject = itemInteraction;
                            break;
                        case CommandUsage.Target:
                            Target = itemInteraction;
                            break;
                    }
        ...
        /// <param name="hasContainer">does the command need a container to look for things in</param>
        /// <param name="seekRange">how far we can look</param>
        public void SeekInUses<T>(CommandPackage command, CommandParameterAttribute currentNeededParm, CommandRangeAttribute seekRange, bool hasContainer)
        {
            IInanimate useThing = null;

            //We should already have a subject or target set so we should have our target
            if (currentNeededParm.Usage == CommandUsage.Target)
                useThing = Subject as IInanimate;
            else if (currentNeededParm.Usage == CommandUsage.Supporting)
                useThing = Target as IInanimate;

            if (useThing == null)
            {
                AccessErrors.Add("That is an invalid item to use.");
                return;
            }
            ...
            while (parmWords > 0)
            {
                ...
                IUse itemInteraction = null;

                itemInteraction = useThing.Template<IInanimateTemplate>().Uses
                        .FirstOrDefault(inte => inte.Name.Equals(currentParmString, StringComparison.InvariantCultureIgnoreCase));
                    ...
                    foreach (IActionCriteria criterion in itemInteraction.Criteria)
                    {
                        ...
                        switch (criterion.Target)
                        {
                            case ActionTarget.Self:
                                ...
                                break;
                            case ActionTarget.Tool:
                                if (!useThing.Qualities.Any(key =>
                                       key.Type == QualityType.Aspect
                                       && (criterion.ValueRange.Low <= 0 || key.Value.IsBetweenOrEqual(criterion.ValueRange.Low, criterion.ValueRange.High))
                                       && key.Name.Equals(criterion.Quality, StringComparison.InvariantCultureIgnoreCase)))
                                {
                                    AccessErrors.Add(string.Format("YOU must have a quality of {1} between {2} and {3}.",
                                        useThing.GetDescribableName(Actor), criterion.Quality, criterion.ValueRange.Low, criterion.ValueRange.High));
                                    return;
                                }
                                break;

                                //We don't check the target here *at all* because the use command code does this for us
                        }
                    }
        ...
        public void SeekInPathway<T>(CommandPackage command, CommandParameterAttribute currentNeededParm, CommandRangeAttribute seekRange, bool hasContainer)
        {
            ...
            while (parmWords > 0)
            {
                ...
                List<IPathwayDestination> validPaths = new List<IPathwayDestination>();

                validPaths.AddRange(Actor.CurrentLocation.GetTile().Pathway()?.Destinations.Where(dest => dest.Name.Equals(currentParmString, StringComparison.InvariantCultureIgnoreCase)));
                ...
                if (validPaths.Count() > 0)
                {
                    ...
                    if (validPaths.Count() > 1)
                    {
                        ...
                        int iterator = 1;
                        foreach (IPathwayDestination obj in validPaths)
                        {
                            IEntity entityObject = (IEntity)obj;
                    ...
                    {
                        IPathwayDestination parm = validPaths.First();
        ...
        /// <param name="commandType">the system type of the command</param>
        /// <param name="currentNeededParm">the conditions for the parameter we're after</param>
        public void SeekInPlayerInventory<T>(CommandPackage command, CommandParameterAttribute currentNeededParm) where T : IEntity
        {
            //Borked it here, we found nothing viable earlier
            if (Actor == null || !((IContains)Actor).GetContents<T>().Any())
                return;

            IEnumerable<T> subjectCollection = ((IContains)Actor).GetContents<T>();
            ...
            while (parmWords > 0)
            {
                ...
                if (validObjects.Count() > 0)
                {
                    ...
                    else if (validObjects.Count() == 1)
                    {
                        ...
                        List<T> validSubjects = new List<T>();

                        validSubjects.AddRange(subjectCollection.Where(sbj => sbj.CurrentLocation.CurrentZone.Equals(container)));
        ...
        public void SeekInTrainerKnowledge<T>(CommandPackage command, CommandParameterAttribute currentNeededParm)
        {
            ...
            while (parmWords > 0)
            {
                ...
                if (typeof(T) == typeof(IUse))
                {
                    validObjects.AddRange(trainer.TeachableAbilities.Where(use => use.Name.Equals(currentParmString, StringComparison.InvariantCultureIgnoreCase))
                                                                    .Select(use => (T)use));
                }
                else if (typeof(T) == typeof(IQuality))
                {
