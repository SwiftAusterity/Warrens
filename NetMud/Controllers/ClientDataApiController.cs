    ...
    public class ClientDataApiController : ApiController
    {
        ...
        public string RenderRoomForEdit(long id, int radius)
        {
            ...
            if (nw != null && !string.IsNullOrWhiteSpace(nw.ToLocationID) && nw.ToLocationType.Equals("Room"))
            {
                ...
                northString += String.Format("<a href='/GameAdmin/EditRoomData/{0}' title='{1}'>#</a>", location.ID, location.Name);
            }
            else
                northString += "O";
            ...
            if (w != null && !string.IsNullOrWhiteSpace(w.ToLocationID) && w.ToLocationType.Equals("Room"))
            {
                ...
                middleString += String.Format("<a href='/GameAdmin/EditRoomData/{0}' title='{1}'>#</a>", location.ID, location.Name);
            }
            else
                middleString += "O";
            ...
            if (sw != null && !string.IsNullOrWhiteSpace(sw.ToLocationID) && sw.ToLocationType.Equals("Room"))
            {
                ...
                southString += String.Format("<a href='/GameAdmin/EditRoomData/{0}' title='{1}'>#</a>", location.ID, location.Name);
            }
            else
                southString += "O";
                                                        ...
                                                        && path.ToLocationType.Equals("Room")))
            {
                ...
                if (location != null)
                    extraString += String.Format(" <a href='/GameAdmin/EditRoomData/{0}' title='{1}'>#</a> ", location.ID, location.Name);
            }
