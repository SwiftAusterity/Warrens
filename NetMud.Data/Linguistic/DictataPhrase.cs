...
namespace NetMud.Data.Linguistic
{
    ...
    public class DictataPhrase : ConfigData, IDictataPhrase, IComparable<IDictataPhrase>, IEquatable<IDictataPhrase>, IEqualityComparer<IDictataPhrase>
    {
        ...
        [JsonProperty("Words")]
        private HashSet<DictataKey> _words { get; set; }
        ...
        public HashSet<IDictata> Words
        {
            get
            {
                if (_words == null)
                {
                    _words = new HashSet<DictataKey>();
                }

                return new HashSet<IDictata>(_words.Select(k => ConfigDataCache.Get<ILexeme>(k.LexemeKey)?.GetForm(k.FormId)));
            }
            set
            {
                if (value == null)
