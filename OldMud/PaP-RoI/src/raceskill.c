...
void do_superform( CHAR_DATA *ch, char *argument )
{
   char arg1[ MAX_INPUT_LENGTH ];
   ...
   if ( ch->race != RACE_HUMAN && ch->race != RACE_AVIASAPIEN
     && ch->race != RACE_CHRONOSAPIEN && ch->race != RACE_AQUASAPIEN )
   {
    typo_message( ch );
    return;
   }
   ...
   if ( arg1[0] == '\0' )
   {
    send_to_char( AT_YELLOW, "Syntax: &Csuperform <type>\n\r"
                             "&WType being: &Caviasapien, chronosapien, aquasapien\n\r", ch );
    send_to_char( AT_YELLOW, "Syntax: &Csuperform revert\n\r", ch );
    return;
   }
   ...
   if ( !str_cmp( arg1, "revert" ))
   {
    ...
    {
     send_to_char( AT_YELLOW, "You already are reverted.\n\r", ch );
     return;
    }

    send_to_char( AT_WHITE, "You revert to your original form.\n\r", ch );
    act( AT_WHITE, "$n appears to have changed.", ch, NULL, NULL, TO_ROOM );
    polymorph_char( ch, get_race_data(ch->race), get_race_data( RACE_HUMAN ) );
    ch->race = RACE_HUMAN;
    return;
   }
    ...
    {
     send_to_char(C_DEFAULT, "You must be at your maximum energy level to transform.\n\r", ch );
     return;
    }

   if ( !str_cmp( arg1, "aviasapien" ) )
   {
    if ( ch->race != RACE_HUMAN )
    {
     send_to_char( AT_YELLOW, "You must be in human form to transform.\n\r", ch );
     return;
    }

    send_to_char( AT_WHITE, "A giant tornado comes down from the sky and envelops you.\n\r"
			          "You emerge as a winged denizen.\n\r", ch );
    act( AT_WHITE, "A giant tornado touches down, enveloping $n.", ch, NULL, NULL, TO_ROOM );
    act( AT_WHITE, "$n emerges; wings now adorn $s back.", ch, NULL, NULL, TO_ROOM );
    polymorph_char( ch, get_race_data(ch->race), get_race_data( RACE_AVIASAPIEN ) );
    ch->race = RACE_AVIASAPIEN;
    return;
   }

   if ( !str_cmp( arg1, "chronosapien" ))
   {
    if ( ch->race != RACE_HUMAN )
    {
     send_to_char( AT_YELLOW, "You must be in human form to transform.\n\r", ch );
    ...
    send_to_char( AT_WHITE, "The sky explodes into a flash of multicoloured lights.\n\r"
			          "You emerge changed.\n\r", ch );
    act( AT_WHITE, "The sky explodes into a flash of multicoloured lights.", ch, NULL, NULL, TO_ROOM );
    act( AT_WHITE, "$n appears changed.", ch, NULL, NULL, TO_ROOM );
    polymorph_char( ch, get_race_data(ch->race), get_race_data( RACE_CHRONOSAPIEN ) );
    ch->race = RACE_CHRONOSAPIEN;
    return;
   }

   if ( !str_cmp( arg1, "aquasapien" ))
   {
    if ( ch->race != RACE_HUMAN )
    {
     send_to_char( AT_YELLOW, "You must be in human form to transform.\n\r", ch );
    ...
    send_to_char( AT_WHITE, "A column of water comes down from the sky and envelops you.\n\r"
			          "You emerge as a denizen of the seas.\n\r", ch );
    act( AT_WHITE, "A giant column of water swirls down from the sky, drowning $n.", ch, NULL, NULL, TO_ROOM );
    act( AT_WHITE, "$n emerges; $s neck appears to have acquired gills.", ch, NULL, NULL, TO_ROOM );
    polymorph_char( ch, get_race_data(ch->race), get_race_data( RACE_AQUASAPIEN )  );
...
void do_headbutt( CHAR_DATA *ch, char *argument )
{
   ...
   {
   case CLASS_WARLOCK:
   case CLASS_HERETIC:      timer = 240;  break;
   case CLASS_THIEF:       timer = 180;  break;
   case CLASS_MURDERER:     timer = 120;  break;
   case CLASS_PAGAN:       timer = 240;  break;
   case CLASS_CHAOMANCER: timer = 240;  break;
   case CLASS_CONMAN:    timer = 180;  break;
   case CLASS_OCCULTIST:    timer = 180;  break;
   case CLASS_TERRORIST:    timer = 180;  break;
   }
   mod = ch->level / 5;
   mod = UMAX( 1, mod );
   timer = timer / mod;
