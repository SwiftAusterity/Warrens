...
void do_wizhelp( CHAR_DATA *ch, char *argument )
{
  ...
  for ( cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++ )
  {
    ...
    {
      clr = AT_GREEN;
    if ( cmd_table[cmd].level >= L_DEM )
    {
      clr = AT_WHITE;
    if ( cmd_table[cmd].level >= L_GOD )
    {
      clr = AT_LBLUE;
    if ( cmd_table[cmd].level >= L_SEN )
    {
      clr = AT_PINK;
    if ( cmd_table[cmd].level >= L_DIR )
    {
      clr = AT_RED;
    if ( cmd_table[cmd].level >= L_CON )
    {
      clr = AT_YELLOW;
    }}}}}}
    else
...
void do_avatar( CHAR_DATA *ch, char *argument )
{
   ...
   int		    avatar = 0;
    ...
    {
	send_to_char(AT_WHITE, "What level do you intend to avatar?\n\r", ch );
	return;
    }
   
    avatar = atoi( arg );

    if ( avatar < 0 || avatar >= ch->level )
    {
     send_to_char( AT_YELLOW, "Level must be between 1 and your level.\n\r", ch );
     return;
    }

    ch->pcdata->avatar = avatar;
    send_to_char( AT_GREEN, "Avatar level set.", ch );
 return;
}
...
void do_advance( CHAR_DATA *ch, char *argument )
{
   ...
   if ( level < LEVEL_IMMORTAL )
   {
    ...
    {
    victim->exp = (xp_tolvl( victim ) - (victim->level * 200));
   }
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char(AT_WHITE, "  example: mset self class 1 murderer\n\r", ch );
	return;
    }
	...
	return;
     ...
     if ( !IS_NPC(ch) && !IS_NPC(victim) && ch->level < victim->level )
      {
	...
	value = -1;
      if ( slot < 1 || slot > 3)
	{
	send_to_char( AT_WHITE, "Invalid slot.  Valid slot: 1-3.\n\r", ch );
	return;
	}
...
void do_rebuild (CHAR_DATA *ch, char *argument)
{
    ...
    victim->pcdata->condition[COND_INSOMNIA]    = 0;
    ...
    switch ( class_table[prime_class(victim)].attr_prime )
    {
      ...
      case APPLY_AGI: victim->perm_agi = 16; break;
    } 
