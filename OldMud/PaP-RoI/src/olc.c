...
void do_spedit(CHAR_DATA *ch, char *argument)
{
  ...
  if(command[0] == 'c' && !str_prefix(command, "create"))
  {
    if(argument[0] == '\0')
    {
      editor_send_to_char(C_DEFAULT, "SPedit:  Syntax: spedit create <spell name>\n\r", ch);
      return; 
    }

    if( skill_lookup(argument) < 0 )
    {
      editor_send_to_char(C_DEFAULT, "SPEdit:  Spell/Skill entry already exist.\n\r", ch);
      return;
    }

    editor_send_to_char(C_DEFAULT, "SPEdit:  Spell/Skill online creation not avable.\n\r", ch);
    return;
  }
  else
  {
    ...
    {
      editor_send_to_char(C_DEFAULT, "Spell/skill entry not found.\n\r", ch);
      return;
    }
