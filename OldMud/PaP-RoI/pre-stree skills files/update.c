...
int hit_gain( CHAR_DATA *ch )
{
    ...
    if ( ch->level < LEVEL_IMMORTAL && (is_class( ch, CLASS_HERETIC ) ||
     is_class( ch, CLASS_PAGAN ) || is_class( ch, CLASS_OCCULTIST ) ) )
    {
     ch->pcdata->regen -= dice( 2, 5 );
     if ( ch->pcdata->regen < 0 )
      ch->pcdata->regen = 0;
     if ( ch->pcdata->regen <= 50 )
     {
      if ( ch->level <= 10 )
      {
       if ( is_class( ch, CLASS_HERETIC ) )
       send_to_char( AT_YELLOW, "Your faith wanes.(you need to pray)\n\r", ch );
       else if ( is_class( ch, CLASS_PAGAN ) )
       send_to_char( AT_YELLOW, "You feel less connected with nature.(you need to commune)\n\r", ch );
       else if ( is_class( ch, CLASS_OCCULTIST ) )
       send_to_char( AT_YELLOW, "'He' asks you to kill again(you need to bloodbath).\n\r", ch );
      }
      else
      {
       if ( is_class( ch, CLASS_HERETIC ) )
        send_to_char( AT_YELLOW, "Your faith wanes.\n\r", ch );
       else if ( is_class( ch, CLASS_PAGAN ) )
       send_to_char( AT_YELLOW, "You feel less connected with nature.\n\r", ch );
       else if ( is_class( ch, CLASS_OCCULTIST ) )
       send_to_char( AT_YELLOW, "'He' asks you to kill again.\n\r", ch );
      }
     }
    }
