...
void move_char( CHAR_DATA *ch, int door, bool Fall, bool flee ) 
{
   ...
   if ( !IS_NPC(ch) )
   {
    ...
    if ( IS_SET( to_room->room_flags, ROOM_TRAPPED ))
    {
     ...
     {
      if ( !is_class(ch, CLASS_THIEF) && !is_class(ch, CLASS_ASSASSIN)
        && !is_class(ch, CLASS_TERRORIST) )
      {
...
void do_train( CHAR_DATA *ch, char *argument )
{
    ...
    else if ( !str_cmp( arg, "agi" ) )
    {
	if ( class_table[prime_class( ch )].attr_prime == APPLY_AGI )
	    cost    = 2;
	pAbility    = &ch->perm_agi;
	pOutput     = "agility";
    }
    ...
    else if ( !str_cmp( arg, "slash" ) )
    {
 	    cost    = 1;
        bone_flag   = 2;
        pAbility    = &ch->pcdata->weapon[WEAPON_SLASH];
        pOutput     = "weapon class slash confidence";
    }
    ...
    else if ( !str_cmp( arg, "pierce" ) )
    {
 	    cost    = 1;
        bone_flag   = 2;
        pAbility    = &ch->pcdata->weapon[WEAPON_PIERCE];
        pOutput     = "weapon class pierce confidence";
    }
    ...
    else if ( !str_cmp( arg, "bash" ) )
    {
        bone_flag   = 1;
 	    cost    = 2;
        pAbility    = &ch->pcdata->weapon[WEAPON_BASH];
        pOutput     = "weapon class bash confidence";
    }

    else if ( !str_cmp( arg, "tear" ) )
    {
 	    cost    = 1;
        bone_flag   = 2;
        pAbility    = &ch->pcdata->weapon[WEAPON_TEAR];
        pOutput     = "weapon class tear confidence";
    }

    else if ( !str_cmp( arg, "chop" ) )
    {
 	    cost    = 1;
        bone_flag   = 2;
        pAbility    = &ch->pcdata->weapon[WEAPON_CHOP];
        pOutput     = "weapon class chop confidence";
    }
    ...
    else if ( !str_cmp( arg, "exotic" ) )
    {
 	    cost    = 1;
        bone_flag   = 2;
        pAbility    = &ch->pcdata->weapon[WEAPON_EXOTIC];
        pOutput     = "weapon class exotic confidence";
    }
   ...
   /* languages -Flux. */
	...
	if ( ch->perm_str < 18 ) strcat( buf, " &Pstr" );
	if ( ch->perm_int < 18 ) strcat( buf, " &Pint" );
	if ( ch->perm_wis < 18 ) strcat( buf, " &Pwis" );
	if ( ch->perm_dex < 18 ) strcat( buf, " &Pdex" );
	if ( ch->perm_con < 18 ) strcat( buf, " &Pcon" );
	if ( ch->perm_agi < 18 ) strcat( buf, " &Pagi" );

	strcat( buf, " &Php mana move" );

      strcat( buf, "\n\r" );

      strcat( buf, "&cWeapon Confidence: " );

        if ( ch->pcdata->weapon[WEAPON_SLASH] < 50 )
	strcat( buf, " &Cslash" );

        if ( ch->pcdata->weapon[WEAPON_PIERCE] < 50 )
	strcat( buf, " &Cpierce" );

        if ( ch->pcdata->weapon[WEAPON_BASH] < 50 )
	strcat( buf, " &Cbash" );

        if ( ch->pcdata->weapon[WEAPON_TEAR] < 50 )
	strcat( buf, " &Ctear" );

        if ( ch->pcdata->weapon[WEAPON_CHOP] < 50 )
	strcat( buf, " &Cchop" );

        if ( ch->pcdata->weapon[WEAPON_EXOTIC] < 50 )
	strcat( buf, " &Cexotic" );
    ...
    {
	act(AT_CYAN, "Your $T is already at its maximum trainable level.", ch, NULL, pOutput, TO_CHAR );
	return;
    }

    if ( *pAbility >= 50 && bone_flag == 2 )
    {
	act(AT_CYAN, "I can not train you any further with that weapon.", ch, NULL, pOutput, TO_CHAR );
	return;
    }
