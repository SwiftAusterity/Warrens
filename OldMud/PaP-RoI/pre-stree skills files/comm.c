...
#if defined( HOTREBOOT )
void init_descriptor (DESCRIPTOR_DATA *dnew, int desc, bool firstport )
{
...
#if defined( HOTREBOOT )
    init_descriptor (dnew, desc, firstport);
#else
    *dnew		= d_zero;
    dnew->descriptor	= desc;
    dnew->character     = NULL;
	    ...
	    i = buf2; break;
         case 'a' :
            if( ch->level >= 5 )
               ...
               sprintf( buf2, "%s", IS_FRAMED( ch ) ? "framed"
		                  : IS_GUILTY( ch ) ? "guilty" : "los"  );
            i = buf2; break;
         case 'r' :
            if( ch->in_room )
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
   ...
   case CON_GET_NEW_PASSWORD:
    ...
    buf2[0]='\0';
    strcpy( buf2, 
"\n\r                 &z-= &RWhat social atrocities have you committed? &z=-\n\r\n\r" );
    iCount = 0;
    ...
    {
        write_to_buffer( d,
        "That's not a crime.\n\rWhat have you done?", 0 );
        return;
    }

    do_help( ch, (char *)class_table[iClass].who_long );
    write_to_buffer( d, "&WSo, you've done this&w? ", 0 );
    ...
    case 'n': case 'N':
    strcpy( buf2, 
"\n\r                 &z-= &RWhat crimes did you commit? &z=-\n\r\n\r"
);
    iCount = 0;
    for ( iClass = 0; iClass < MAX_CLASS; iClass++ )
    {
  ...
  break;
    ...
    case CON_REMORT_FIRST:
    do_help( ch, "'FIRST REMORT'" );
    sprintf( buf, 
"&WWe just recieved an anonymous tip from Sammy the snitch\n\r"
"that you are more than a %s. Are you gonna come clean punk?\n\r",
			class_table[ch->class[0]].who_long );          
    write_to_buffer( d, buf, 0 );
    d->connected = CON_WANT_REMORT;
    break;

    case CON_WANT_REMORT:
	switch ( *argument )
	  {
	  case 'y': case 'Y':
    strcpy( buf2, 
"\n\r                 &z-= &RWhat crimes have you committed? &z=-"
"\n\r           &cYou may type &RBACK &cto return to previous selection.\n\r\n\r"
	  ...
	  case 'n': case 'N':
            ...
            do_help( ch, "DEPUTY" );
        sprintf( buf,
"&WClammin' up then eh? Well then, are you gonna go straight\n\r"
"or are we going to have to put you on the most wanted list? "  );
            write_to_buffer( d, buf, 0 );
   	    d->connected = CON_GET_DEPUTY;
	    break;
    ...
    {
        write_to_buffer( d,
        "&cThat's not a crime&w.\n\r&WWhat have you done&w? ", 0 );
    d->connected = CON_GET_REMORT;
    break;
    }

    do_help( ch, (char *)class_table[iClass].who_long );
    write_to_buffer( d, "&WAre these the crimes you've committed&w? ", 0 );
    d->connected = CON_CONFIRM_REMORT;
    break;

    case CON_CONFIRM_REMORT:
    switch ( *argument )
    {
    case 'y': case 'Y':
        ch->alignment      = -1000;
...
"\n\r           &cYou may type &RBACK &cto return to previous selection.\n\r\n\r"
    ...
    case 'y': case 'Y':
    write_to_buffer( d, "&WOh yeah&w? ", 0 );
      d->connected = CON_CONFIRM_DEPUTY;
    break;

    case 'n': case 'N':
    write_to_buffer( d, "&WOh yeah&w? ", 0 );
      d->connected = CON_CONFIRM_WANTED;
    break;
...
/* END */
    ...
    if ( ch->level == 0 )
    {
        ...
        case APPLY_AGI: ch->perm_agi = 16; break;
        }
	...
	ch->size		= pRace->size;
        ...
        set_title( ch, buf );
           ch->prompt = str_dup( "<&Y%h&Ohp &C%m&cm &G%v&gmv &P%X&pxp&w> " );
     write_to_buffer(d,
...
"\n\r                  &C*^* &PSelect a race from the list below &C*^*\n\r\n\r" );
    ...
    buf2[0]='\0';
    strcpy( buf2, 
"\n\r                 &C*^* &PWhat profession do you wish to persue? &C*^*"
"\n\r               &cYou may type &RBACK &cto return to previous selection.\n\r\n\r"
);
    iCount = 0;
    for ( iClass = 0; iClass < MAX_CLASS; iClass++ )
...
"\n\r                  &C*^* &PSelect a race from the list below &C*^*\n\r\n\r" );
    ...
    {
        write_to_buffer( d,
        "That is not a proper profession.\n\rWhat is your choice?", 0 );
        return;
    }

    do_help( ch, (char *)class_table[iClass].who_long );
    write_to_buffer( d, "&WSo, you wish to do this&w? ", 0 );
    d->connected = CON_CONFIRM_CLASS;
        break;
    ...
    if(mode == 0)
    {
        ...
        && d->connected <= CON_AUTHORIZE_LOGOUT)
      {
        sprintf(buf, "  %s!%s@%s a %s %s %s %s\n\r",
           ...
           IS_FRAMED( d->character ) ? "framed" :
           IS_GUILTY( d->character ) ? "guilty" : "los",
...
void hotreboot_recover ( )
{
	...
	for (;;)
	{
		...
		init_descriptor (d,desc, TRUE); /* set up various stuff */
		...
		d->connected = CON_HOTREBOOT_RECOVER;
		...
		else /* ok! */
		{
			...
			d->character->in_room = get_room_index(ROOM_HUMAN_TEMPLE);
