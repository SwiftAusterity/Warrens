	...
	break;
     ...
     else
     {
      if ( ch->pcdata->remort == 0 ||
       ( ch->level < LEVEL_IMMORTAL && obj->level > LEVEL_MORTAL ) )
      {
	act( AT_ORANGE, "$p is too high level for you.", ch, obj, NULL,
		 TO_CHAR );
	break;
...
void do_sell( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( !cost )
     || ( cost->gold == 0 && cost->silver == 0 && cost->copper == 0 )
     || obj->level >= LEVEL_IMMORTAL || obj->timer >= 0)
    {
...
void do_quiverpalm( CHAR_DATA *ch, char *argument )
{
  AFFECT_DATA af;
  if ( is_affected( ch, gsn_quiverpalm ) )
	return;
  if ( IS_NPC( ch ) || 
       !can_use_skpell( ch, gsn_quiverpalm ) )
	{
        typo_message( ch );
	return;
	}

  if ( ch->pcdata->learned[gsn_quiverpalm] < number_percent( ) )
	{
	send_to_char( AT_GREY, "You cannot seem to control your chi.\n\r",
		      ch );
	return;
	}
  af.type	 = gsn_quiverpalm;
  af.level	 = ch->level;
  af.duration	 = 20 + ( 5 * (ch->level > 75) ) + ( 10 * (ch->level >95) );
  af.location	 = APPLY_NONE;
  af.modifier	 = 0;
  ...
  act( AT_GREY, "$n goes into a trance, $s palms facing to the sky.",
       ch, NULL, NULL, TO_ROOM );
  send_to_char( AT_GREY, "Your hands start to vibrate.\n\r", ch );
  act( AT_GREY, "$n's hands start to vibrate.",
       ch, NULL, NULL, TO_ROOM );
  update_skpell( ch, gsn_quiverpalm );
  return;
}
...
void do_shockhand( CHAR_DATA *ch, char *argument )
{
  AFFECT_DATA af;
  if ( is_affected( ch, gsn_shockhand ) )
        return;
  if ( IS_NPC( ch ) ||
  !can_use_skpell( ch, gsn_shockhand ) )
  {
  typo_message( ch );
  return;
  }

  if ( ch->pcdata->learned[gsn_shockhand] < number_percent( ) )
        {
        send_to_char( AT_GREY, "You cannont seem to control your chi.\n\r",  
                      ch );
        return;
        }
 af.type      = gsn_shockhand;
 af.level     = ch->level;
 af.duration  = 20 + ( 5 * (ch->level > 90) )  + ( 10 * (ch->level >= 100) );
 af.location  = APPLY_NONE;
 af.modifier  = 0;
 ...
 send_to_char( AT_GREY, "You tap deep down into your chi.\n\r", ch );
  act( AT_GREY, "$n goes into a trance, $s palms facing to the sky.",
       ch, NULL, NULL, TO_ROOM );
  send_to_char( AT_GREY, "Your hands sparkle with &Yelectricity&w.\n\r", ch );
  act( AT_GREY, "$n's hands start to sparkle with &Yelectricity&w.",
       ch, NULL, NULL, TO_ROOM );
  update_skpell( ch, gsn_shockhand );
  return;
}
void do_colorhand( CHAR_DATA *ch, char *argument )
{
  AFFECT_DATA af;
  if ( is_affected( ch, gsn_colorhand ) )
        return;
  if ( IS_NPC( ch ) ||
  !can_use_skpell( ch, gsn_colorhand ) )
  {
  typo_message( ch );
  return;
  }

  if ( ch->pcdata->learned[gsn_colorhand] < number_percent( ) )
        {
        send_to_char( AT_GREY, "You cannont seem to control your chi.\n\r",  
                      ch );
        return;
        }
 af.type      = gsn_colorhand;
 af.level     = ch->level;
 af.duration  = 20 + ( 5 * (ch->level > 90) )  + ( 10 * (ch->level >= 100) );
 af.location  = APPLY_NONE;
 af.modifier  = 0;
 ...
 send_to_char( AT_GREY, "You tap deep down into your chi.\n\r", ch );
  act( AT_GREY, "$n goes into a trance, $s palms facing to the sky.",
       ch, NULL, NULL, TO_ROOM );
  send_to_char( AT_GREY, "&WA &Rr&Oa&Yi&Gn&Bb&Co&Pw &Wwraps around your hands&w.\n\r", ch );
  act( AT_GREY, "&WA &Rr&Oa&Yi&Gn&Bb&Co&Pw&W &Wwraps around $n's hands&w.",
       ch, NULL, NULL, TO_ROOM );
  update_skpell( ch, gsn_colorhand );
  return;
}
void do_darkhand( CHAR_DATA *ch, char *argument )
{
  AFFECT_DATA af;
  if ( is_affected( ch, gsn_darkhand ) )
        return;
  if ( IS_NPC( ch ) ||
  !can_use_skpell( ch, gsn_darkhand ) )
  {
  typo_message( ch );
  return;
  }

  if ( ch->pcdata->learned[gsn_darkhand] < number_percent( ) )
        {
        send_to_char( AT_GREY, "You cannont seem to control your chi.\n\r",  
                      ch );
        return;
        }
 af.type      = gsn_darkhand;
 af.level     = ch->level;
 af.duration  = 20 + ( 5 * (ch->level > 90) )  + ( 10 * (ch->level >= 100) );
 af.location  = APPLY_NONE;
 af.modifier  = 0;
 ...
 send_to_char( AT_GREY, "You tap deep down into your chi.\n\r", ch );
  act( AT_GREY, "$n goes into a trance, $s palms facing to the sky.",
       ch, NULL, NULL, TO_ROOM );
  send_to_char( AT_GREY, "Your hands become shrouded in darkness&w.\n\r", ch );
  act( AT_GREY, "$n's hands become shrouded in darkness&w.",
       ch, NULL, NULL, TO_ROOM );
  update_skpell( ch, gsn_darkhand );
  return;
}
...
void do_molotov( CHAR_DATA *ch, char *argument )
{
  OBJ_DATA *flask;
  OBJ_DATA *chemical;
  OBJ_DATA *wick;
  ...
  if ( !flask )
    {
    send_to_char( AT_GREY, "You need a beaker to make a molotov cocktail.\n\r", ch );
    return;
    }

  /* check to see if we have the chemical. */
  for ( chemical = ch->carrying; chemical; chemical = chemical->next_content )
   {
     if ( !chemical )
      continue;

    if ( chemical->durability <= 0 )
     continue;

   if ( chemical->item_type == ITEM_MOLOTOVCHEM )
	break;
   }

  if ( !chemical )
   {
   send_to_char( AT_GREY, "You need a chemical to make that bomb.\n\r", ch );
   return;
   }

  /* check to see if we have the wick. */
  for ( wick = ch->carrying; wick; wick = wick->next_content )
   {
     if ( !wick )
      continue;
    
    if ( wick->durability <= 0 )
     continue;

   if ( wick->item_type == ITEM_WICK )
	break;
   }

  if ( !wick )
   {
   send_to_char( AT_GREY, "You need a wick to make that bomb.\n\r", ch );
   return;
   }
	...
	"Your hands aren't steady enough to properly mix the chemicals.\n\r",
								ch );
	return;
    }

    /* Check the skill percentage */
    if ( number_percent( ) > ch->pcdata->learned[gsn_molotov] )
    {
	send_to_char(AT_DGREEN, "You failed and your mistake explodes! Ouch!\n\r",
		     ch );
	damage( ch, ch, ch->level, gsn_molotov, DAM_HEAT, TRUE  );
	act(AT_DGREEN, "$n's concotion explodes!", ch, NULL, NULL, TO_ROOM );
        extract_obj( chemical );
        extract_obj( wick );
        extract_obj( flask );
	return;
    }

  WAIT_STATE( ch, skill_table[gsn_molotov].beats );

  flask->level = ch->level;
  flask->item_type = ITEM_BOMB;
  flask->value[1] = BOMB_NAPALM;
  flask->value[3] = ch->level + ch->pcdata->learned[gsn_molotov];
  flask->cost.gold = 25;
  free_string( flask->name );
  flask->name	  = str_dup( "molotov cocktail" );
  free_string( flask->short_descr );
  flask->short_descr = str_dup( "A molotov cocktail" );
  free_string( flask->description );
  flask->description  = str_dup( "A small flask filled with a clear liquid sets upon the floor." );
  send_to_char( AT_GREY, "You mix a molotov cocktail.\n\r", ch );
  act( AT_GREY, "$n pours some liquid into an empty flask.\n\r",
       ch, NULL, NULL, TO_ROOM );
  update_skpell( ch, gsn_molotov );
  extract_obj( chemical );
  extract_obj( wick );
  return;
}

void do_pipebomb( CHAR_DATA *ch, char *argument )
{
  OBJ_DATA *pilewire;
  OBJ_DATA *chemical;

  if ( IS_NPC( ch ) 
  || !can_use_skpell( ch, gsn_pipebomb ) )
    {
    typo_message( ch );
    return;
    }
  ...
  if ( !pilewire )
    {
    send_to_char( AT_GREY, "You need a pile of wires to make a pipe bomb.\n\r", ch );
    return;
    }
  ...
  for ( chemical = ch->carrying; chemical; chemical = chemical->next_content )
   {
    ...
    if ( chemical->item_type == ITEM_PIPEBOMBCHEM )
	break;
   }

  if ( !chemical )
   {
   send_to_char( AT_GREY, "You need plastique to make that bomb.\n\r", ch );
   return;
   }
    ...
    /* Check the skill percentage */
    if ( number_percent( ) > ch->pcdata->learned[gsn_pipebomb] )
    {
	send_to_char(AT_DGREEN, "You failed and your mistake explodes! Ouch!\n\r",
		     ch );
	damage( ch, ch, ch->level, gsn_pipebomb, DAM_HEAT, TRUE  );
	act(AT_DGREEN, "$n's concotion explodes!", ch, NULL, NULL, TO_ROOM );
        extract_obj( chemical );
        extract_obj( pilewire );
	return;
    ...
    WAIT_STATE( ch, skill_table[gsn_pipebomb].beats );


  chemical->level = ch->level;
  chemical->item_type = ITEM_BOMB;
  chemical->value[1] = BOMB_EXPLOSIVE;
  chemical->value[3] = (ch->level*2) + (ch->pcdata->learned[gsn_pipebomb]*2);
  chemical->cost.gold = 35;
  free_string( chemical->name );
  chemical->name	  = str_dup( "pipe bomb" );
  free_string( chemical->short_descr );
  chemical->short_descr = str_dup( "A pipe bomb" );
  free_string( chemical->description );
  chemical->description  = str_dup( "A small grey block with wires pushed into it sits upon the floor." );
  send_to_char( AT_GREY, "You make a pipe bomb.\n\r", ch );
  act( AT_GREY, "$n connects some wires into a block of grey clay.\n\r",
       ch, NULL, NULL, TO_ROOM );
  update_skpell( ch, gsn_pipebomb );
  extract_obj( pilewire );
  return;
}

void do_cherrybomb( CHAR_DATA *ch, char *argument )
{
  OBJ_DATA *container;
  OBJ_DATA *chemical;

  if ( IS_NPC( ch ) 
  || !can_use_skpell( ch, gsn_cherrybomb ) )
    {
    typo_message( ch );
    return;
    }
  ...
  for ( container = ch->carrying; container; container = container->next_content )
   ...
   if ( container->item_type == ITEM_CHERRYCONTAINER )
	break;
   }

  if ( !container )
    {
    send_to_char( AT_GREY, "You need a small red ball to make a cherry bomb.\n\r", ch );
    return;
    }
  ...
  for ( chemical = ch->carrying; chemical; chemical = chemical->next_content )
   {
    ...
    if ( chemical->item_type == ITEM_GUNPOWDER )
	break;
   }

  if ( !chemical )
   {
   send_to_char( AT_GREY, "You need black powder to make that bomb.\n\r", ch );
   return;
   }
    ...
    /* Check the skill percentage */
    if ( number_percent( ) > ch->pcdata->learned[gsn_cherrybomb] )
    {
	send_to_char(AT_DGREEN, "You failed and your mistake explodes! Ouch!\n\r",
		     ch );
	damage( ch, ch, ch->level, gsn_cherrybomb, DAM_HEAT, TRUE  );
	act(AT_DGREEN, "$n's concotion explodes!", ch, NULL, NULL, TO_ROOM );
        extract_obj( chemical );
    ...
    WAIT_STATE( ch, skill_table[gsn_cherrybomb].beats );


  container->level = ch->level;
  container->item_type = ITEM_BOMB;
  container->value[1] = BOMB_CHERRY;
  container->value[3] = ch->pcdata->learned[gsn_cherrybomb] + ch->level;
  container->cost.gold = 10;
  free_string( container->name );
  container->name	  = str_dup( "cherry bomb" );
  free_string( container->short_descr );
  container->short_descr = str_dup( "A cherry bomb" );
  free_string( container->description );
  container->description  =
  str_dup( "A small red ball with a wick sits upon the floor." );
  send_to_char( AT_GREY, "You make a cherry bomb.\n\r", ch );
  act( AT_GREY, "$n pours some powder into a container.\n\r",
       ch, NULL, NULL, TO_ROOM );
  update_skpell( ch, gsn_cherrybomb );
  extract_obj( chemical );
  return;
}

void do_teargas( CHAR_DATA *ch, char *argument )
{
  OBJ_DATA *flask;
  OBJ_DATA *chemical;
  if ( IS_NPC( ch ) 
  || !can_use_skpell( ch, gsn_teargas ) )
    {
    typo_message( ch );
    return;
    }
  ...
  if ( !flask )
    {
    send_to_char( AT_GREY, "You need a beaker to make a tear gas bomb.\n\r", ch );
    return;
    }
  ...
  for ( chemical = ch->carrying; chemical; chemical = chemical->next_content )
   ...
   if ( chemical->item_type == ITEM_TEARCHEM )
	break;
   }
  if ( !chemical )
   {
   ...
   return;
    ...
    /* Check the skill percentage */
    if ( number_percent( ) > ch->pcdata->learned[gsn_teargas] )
    {
	send_to_char(AT_DGREEN, "You failed and your mistake explodes! Ouch!\n\r",
		     ch );
	damage( ch, ch, ch->level, gsn_teargas, DAM_DEGEN, TRUE  );
	act(AT_DGREEN, "$n's concotion explodes!", ch, NULL, NULL, TO_ROOM );
        extract_obj( chemical );
        extract_obj( flask );
	return;
    ...
    WAIT_STATE( ch, skill_table[gsn_teargas].beats );

  flask->level = ch->level;
  flask->item_type = ITEM_BOMB;
  flask->value[1] = BOMB_TEAR;
  flask->value[3] = ch->level + ch->pcdata->learned[gsn_teargas];
  flask->cost.gold = 0;
  free_string( flask->name );
  flask->name	  = str_dup( "tear gas bottle" );
  free_string( flask->short_descr );
  flask->short_descr = str_dup( "A bottle of tear gas" );
  free_string( flask->description );
  flask->description  = str_dup( "A small flask filled with a clear liquid sets upon the floor." );
  send_to_char( AT_GREY, "You mix a tear gas bomb.\n\r", ch );
  act( AT_GREY, "$n pours some liquid into an empty flask.\n\r",
       ch, NULL, NULL, TO_ROOM );
  update_skpell( ch, gsn_teargas );
  extract_obj( chemical );
  return;
}
void do_chemicalbomb( CHAR_DATA *ch, char *argument )
{
  OBJ_DATA *flask;
  OBJ_DATA *chemical;
  if ( IS_NPC( ch ) 
  || !can_use_skpell( ch, gsn_chemicalbomb ) )
    {
    typo_message( ch );
    return;
    }
  ...
  if ( !flask )
    {
    send_to_char( AT_GREY, "You need a beaker to make a chemical bomb.\n\r", ch );
    return;
    }
  ...
  for ( chemical = ch->carrying; chemical; chemical = chemical->next_content )
   ...
   if ( chemical->item_type == ITEM_CHEMBOMBCHEM )
	break;
   }
  if ( !chemical )
   {
   ...
   return;
    ...
    /* Check the skill percentage */
    if ( number_percent( ) > ch->pcdata->learned[gsn_chemicalbomb] )
    {
	send_to_char(AT_DGREEN, "You failed and your mistake explodes! Ouch!\n\r",
		     ch );
	damage( ch, ch, ch->level, gsn_chemicalbomb, DAM_DEGEN, TRUE  );
	act(AT_DGREEN, "$n's concotion explodes!", ch, NULL, NULL, TO_ROOM );
        extract_obj( chemical );
        extract_obj( flask );
	return;
    ...
    WAIT_STATE( ch, skill_table[gsn_chemicalbomb].beats );

  flask->level = ch->level;
  flask->item_type = ITEM_BOMB;
  flask->value[1] = BOMB_CHEMICAL;
  flask->value[3] = (ch->level*2) + (ch->pcdata->learned[gsn_chemicalbomb]*3);
  flask->cost.gold = 0;
  free_string( flask->name );
  flask->name	  = str_dup( "chemical bomb" );
  free_string( flask->short_descr );
  flask->short_descr = str_dup( "A chemical bomb" );
  free_string( flask->description );
  flask->description  = str_dup( "A small flask filled with a clear liquid sets upon the floor." );
  send_to_char( AT_GREY, "You mix a chemical bomb.\n\r", ch );
  act( AT_GREY, "$n pours some liquid into an empty flask.\n\r",
       ch, NULL, NULL, TO_ROOM );
  update_skpell( ch, gsn_chemicalbomb );
  extract_obj( chemical );
  return;
}
