  ...
  if ( !can_use_skpell( ch, gsn_disguise ) )
  {
   ...
   return;
	...
	return;
      ...
      if (ch->level < L_APP && victim->level > LEVEL_MORTAL )
      {
...
"/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\n\r", ch );  
	...
	if ( wch->level >= LEVEL_HERO && wch->level < LEVEL_IMMORTAL )
	  num_of_champ++;
	...
	if ( wch->level >= LEVEL_HERO )
	    switch ( wch->level )
	      {
	      default: break;
              case L_IMP: class = wch->sex == 2 ? "&PWardeness&W"
						: "&RWarden&W"; break;
              case L_CON: class = "&GWarden&W"; break;
	      case L_DIR: class = "&BWarden&W"; break;
              case L_GOD: class = "&YWarden&W"; break;
	      case L_SEN: class = "&CWarden&W"; break;
              case L_DEM: class = "&WWarden";break;
              case L_JUN: class = "&zWarden&W"; break;
	      case L_APP: class = "&RDeputy&W"; break;
	      case L_CHAMP5:
	      case L_CHAMP4:
	      case L_CHAMP3:
	      case L_CHAMP2:
        ...
        if (wch->clan != 0)
          {
           ...
           if ( IS_SET(pClan->settings, CLAN_PKILL) )
              switch ( wch->clev )
              {
               ...
               case 5:
                  sprintf( clan, "-<Warden of %s>-", pClan->name ); break;
              }
           else
              switch ( wch->clev )
              {
               ...
               case 5:
                  sprintf( clan, "(Warden of %s&C)", pClan->name ); break;
              }
 	}
       }
 	/*
	...
	if ( wch->level < 10 )
	  {
          if ( wch->sex == 0 )
	  sprintf( buf + strlen( buf ), "[&G  %d %s N  &W] ",
	  	   wch->level, race );
          if ( wch->sex == 1 )
	  sprintf( buf + strlen( buf ), "[&G  %d %s M  &W] ",
	  	   wch->level, race );
          if ( wch->sex == 2 )
	  sprintf( buf + strlen( buf ), "[&G  %d %s F  &W] ",
	  	   wch->level, race );
	  }
	if ( wch->level < 100 && wch->level > 9 )
	  {
          if ( wch->sex == 0 )
          {
	  sprintf( buf + strlen( buf ), "[&C  %d %s N &W] ",
	  	   wch->level, race );
          }
          if ( wch->sex == 1 )
          {
	  sprintf( buf + strlen( buf ), "[&C  %d %s M &W] ",
	  	   wch->level, race );
          }
          if ( wch->sex == 2 )
          {
	  sprintf( buf + strlen( buf ), "[&C  %d %s F &W] ",
	  	   wch->level, race );
	  }
         }
	if ( wch->level < LEVEL_HERO && wch->level > 99 )
	  {
          if ( wch->sex == 0 )
          {
	  sprintf( buf + strlen( buf ), "[ &P%d %s N &W] ",
	  	   wch->level, race );
          }
          if ( wch->sex == 1 )
          {
	  sprintf( buf + strlen( buf ), "[ &P%d %s M &W] ",
	  	   wch->level, race );
          }
          if ( wch->sex == 2 )  
	  sprintf( buf + strlen( buf ), "[ &P%d %s F &W] ",
	  	   wch->level, race );
	 }
        }
...
"\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\n\r", ch );
   ...
   if ( ch->pcdata->port == PORT_PAP )
   {
    sprintf( buf, "\n\r&RWardens&r[&R%d&r] &GMost Wanted&g[&G%d&g]&G\n\r",
     num_of_imm, num_of_champ );
    send_to_char ( AT_RED, buf, ch );
   ...
   {
    sprintf( buf, "\n\r&RImmortals&r[&R%d&r] &GHeroes&g[&G%d&g]&G\n\r",
     num_of_imm, num_of_champ );
    send_to_char ( AT_RED, buf, ch );
  ...
  send_to_char( AT_GREEN, "There is no class/race/clan/person by that name in the paradox.\n\r", ch );
  return;
}
...
void do_where( CHAR_DATA *ch, char *argument )
{
	...
	{      	    
		...
		|| victim->race == RACE_KENDER ) && ch->level < LEVEL_IMMORTAL) 
...
void do_slist ( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( ch ) )
    {  
       send_to_char (AT_BLUE, "You don't need any stinking spells!\n\r", ch );
       return;
    }

    send_to_char( AT_BLUE, "&wLv            &PS&Gp&Pe&Gl&Pl&Gs&W/&CS&ck&Bi&bl&Cl&cs\n\r\n\r", ch );
	...
	if ( !skill_table[sn].name )
	  ...
	  continue;
        ...
        if ( classes == 2 )
        {
         if ( ( skill_table[sn].dualclass_one != profession
                && skill_table[sn].dualclass_one != ch->class[1] )
             || ( skill_table[sn].dualclass_two !=  profession
                && skill_table[sn].dualclass_two != ch->class[1] )
             || skill_table[sn].dualclass_level != level )
          if ( skill_table[sn].skill_level[profession] != level )
           if ( skill_table[sn].skill_level[ch->class[1]] != level )
          continue;
        }
	...
	if ( ++col % 4 == 0 )
	  send_to_char( AT_BLUE, "   ", ch );
	   ...
	   skill_table[sn].touch_fun == touch_null )
       {
        if ( skill_table[sn].target == TAR_IGNORE ) 
        {
         if ( skill_table[sn].minimum_position == POS_RESTING )
         {
          if ( level < 100 )
 	   sprintf ( buf, "&B%24s", skill_table[sn].name );
          else
           sprintf ( buf, "&B%23s", skill_table[sn].name );
         }
         else if ( skill_table[sn].minimum_position == POS_STANDING ) 
         {
          if ( level < 100 )
  	   sprintf ( buf, "&b%24s", skill_table[sn].name );
          else
 	   ...
 	   sprintf ( buf, "&c%24s", skill_table[sn].name );
           ...
           sprintf ( buf, "&c%23s", skill_table[sn].name );
         }
        }
       ...
       else if( skill_table[sn].touch_fun == touch_null )
       {
        if ( level < 100 )
 	 sprintf ( buf, "&G%24s", skill_table[sn].name );
        else
         sprintf ( buf, "&G%23s", skill_table[sn].name );
       }
       else if( skill_table[sn].spell_fun == spell_null )
       {
        if ( level < 100 )
 	 sprintf ( buf, "&P%24s", skill_table[sn].name );
        else
         sprintf ( buf, "&P%23s", skill_table[sn].name );
       }
...
, "Races" );
  ...
  send_to_char( C_DEFAULT, buf, ch );
    ...
    for ( pRace  = first_race;
          ...
          pRace  = pRace->next )
    {
     if ( pRace->polymorph == TRUE && ch->level < LEVEL_IMMORTAL)
     continue;
...
void do_worth( CHAR_DATA *ch, char *argument )
{
  ...
  sprintf( log_buf, "Your current stats are:\n\rStr: &C%d&R  Wis: &C%d&R  Int: &C%d&R  Dex: &C%d&R  Con: &C%d&R  Agi: &C%d&R Cha: &C%d&R\n\r",
    get_curr_str( ch ), get_curr_wis( ch ), get_curr_int( ch ),
    get_curr_dex( ch ), get_curr_con( ch ), get_curr_agi( ch ),
    get_curr_cha( ch ) );
  send_to_char( AT_RED, log_buf, ch );

  sprintf( log_buf, "You are carrying &Y%d &Rgold, &w%d &Rsilver, and &O%d &Rcopper coins.\n\r",
            ch->money.gold, ch->money.silver, ch->money.copper );
  send_to_char( AT_RED, log_buf, ch );

    sprintf( log_buf, "Flying:    %3s Invis:     %3s  Sneak:     %3s  Hide:     %3s\n\r",
	    ( is_flying(ch) ) ? "&Gyes&R"
	                                   : "&Bno &R",
	    ( IS_SET( ch->affected_by, AFF_INVISIBLE   ) ) ? "&Gyes&R"
	                                   : "&Bno &R",
	    ( IS_SET( ch->affected_by, AFF_SNEAK  ) ) ? "&Gyes&R"
	                                   : "&Bno &R",
	    ( IS_SET( ch->affected_by, AFF_HIDE  ) ) ? "&Gyes&R"
	                                   : "&Bno &R" );
    send_to_char( AT_RED, log_buf, ch );    
...
void do_finger( CHAR_DATA *ch, char *argument )
{
  ...
  sprintf( buf, "&CLevel: &W%3d           &CSex:  &W%s\n\r",
                victim->level,
...
void do_walkmes(CHAR_DATA *ch, char *argument )
{
   ...
   if ( ch->pcdata->remort == 0 && ch->level <= LEVEL_MORTAL )
   {
    send_to_char( AT_WHITE, "Silly mortal, walkmessages are only for remorts.\n\r", ch );
    return;
   }
...
void do_immlist( CHAR_DATA *ch, char *argument )
{
 ...
 send_to_char( AT_BLUE, "\b\rWardens of your world\n\r" , ch );
